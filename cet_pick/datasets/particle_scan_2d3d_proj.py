...
class ParticleSCAN2D3DProjDataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		# tomo = self.tomos[i]
		sub_vol = torch.from_numpy(self.subvols[index])
		neighbor_indices = self.indices[index]
		neighbor_index = np.random.choice(neighbor_indices, 1)[0]
		neighbor = torch.from_numpy(self.subvols[neighbor_index])

		# aug_anchor = self.transforms(sub_vol)
		aug_anchor = self.weak_transforms(sub_vol)
		aug_neighbor = self.transforms(neighbor)

		ret = {'anchor_2d': aug_anchor[0:1], 'anchor_3d': aug_anchor[1:2], 'neighbor_2d': aug_neighbor[0:1], 'neighbor_3d': aug_neighbor[1:2],'possible_neighbors': torch.from_numpy(neighbor_indices)}
		# ret['']
		...
		# ret = {'input': aug_1.astype(np.float32), 'input_aug': aug_2.astype(np.float32)}
