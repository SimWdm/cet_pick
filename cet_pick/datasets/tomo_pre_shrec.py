...
class TOMOPreSHREC(Dataset):
	num_classes = 1 
	default_resolution = [256, 256]

	def __init__(self, opt, split, size):
		super(TOMOPreSHREC, self).__init__()
		# if split == 'train':
		...
		# self.tomos, self.names, self.sub_vols = self.load_data()
		self.tomos, self.hms, self.gt_dets, self.names, self.sub_vols, self.labels = self.load_data()
		# self.num_samples = len(self.tomos)
		self.num_samples = len(self.sub_vols)
		print('num_of_samples', self.num_samples)
		print('num_of_tomos', len(self.tomos))
		self.transforms = tio.Compose([
			...
			tio.transforms.RandomGamma(log_gamma=(-0.5, 0.5),p=0.5),
	...
	def load_data(self, image_ext=''):
		...
		images = load_tomos_from_list(image_list.image_name, image_list.path,order='zxy', compress=False, denoise=True)
	...
	# def load_data(self, image_ext=''):
		...
		for i in range(num_of_tomos):
			...
			sample = random.sample(list(np.arange(num_negs)), len(lbs)*2)
			...
			# 	subvols.append(cropped_subvol)
			# 	lbs.append(0)
			# print('with neg subvol', len(subvols))
			tomos.append(tomo)
			hms.append(hm)
			# inds.append(ind)
	...
	# 	image_list = pd.read_csv(self.data_dir, sep='\t')
	# 	# coords = pd.read_csv(self.coord_dir, sep='\t')
	# 	images = load_tomos_from_list(image_list.image_name, image_list.path, order='zxy')
	# 	# num_particles = len(coords)
