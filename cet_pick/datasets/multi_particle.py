...
class MultiParticleDataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		if self.split == 'train':
			hm = np.zeros((num_class, depth, output_h, output_w), dtype=np.float32) - 1
		else:
			hm = np.zeros((num_class, depth, output_h, output_w), dtype=np.float32)
		...
		for k in range(num_objs):
			...
			reg_mask[k] = 1
			gt_det.append([ct_int[0], ct_int[1], ct_int[2], cls_id])

			if self.opt.contrastive:
				offs_x_y = [-4,-3, -2, 2, 3, 4]
				offs_z = [-2, -1, 1, 2]
			...
			gt_det = np.array(gt_det, dtype=np.float32) if len(gt_det) > 0 else np.zeros((1,4), dtype=np.float32)
