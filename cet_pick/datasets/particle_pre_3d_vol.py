...
class ParticlePre3DVolDataset(data.Dataset):
    ...
    def extract_patches(self, v, tomo_coord, angles, tomo_size=[512, 512, 256]):
        ...
        for ind, an in enumerate(angles):
            ...
            if tx <= self.size[1]//2 or tx >= self.tomo_size[1]-self.size[0]//2 or ty <= self.size[2]//2 or ty >= self.tomo_size[1]-self.size[1]//2:
                        continue 
    ...
    def __getitem__(self, index):
        ...
        aug_1 = self.transforms(sub_vol3d)
       
        ret = {'input': aug_1.float(), 'input_aug': aug_2.float(), 'coord':curr_coord}
