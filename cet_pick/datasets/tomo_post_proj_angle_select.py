...
from cet_pick.utils.image import gaussian_radius, draw_umich_gaussian_3d, draw_msra_gaussian_3d, flip_ud, flip_lr, CornerErasing, CenterOut,AdjustBrightness,InvertColor,FixedRotation
import torchio as tio 
from multiprocessing import Pool


class TOMOPostProjAngleSelect3DVol(Dataset):
    num_classes = 1 
    default_resolution = [256, 256]

    def __init__(self, opt, split, size, low = -20, up = 20, sigma1=2, sigma2=4, K=5000):
        super(TOMOPostProjAngleSelect3DVol, self).__init__()
        if split == 'train':
        ...
        ### This is for 3D transform augmentation - something you will prob switch to
        ...
        self.transforms = T.Compose([
            ...
            T.RandomResizedCrop(self.size[1], (0.7,1.0),(1.0,1.0)),
    ...
    def load_data(self, image_ext=''):
        ...
        orig_recs = load_tomos_from_list(image_list.image_name, image_list.path,compress=self.opt.compress, denoise=self.opt.gauss)
        coords = pd.read_csv(self.coord_dir, sep='\t')
        num_particles = len(coords)
        ims, targets, names = self.match_images_targets(orig_recs, coords)
                ...
                if tomo_x > self.crop_size_x//1.8 and tomo_x < self.tomo_size[1]-self.crop_size_x//1.8 and tomo_y >= self.crop_size_y//1.8 and tomo_y <= self.tomo_size[1]-self.crop_size_y//1.8:
                    ...
                    if self.split == 'train':
                        ...
                        if len(patch3d_sets) > 1:
                            ...
                            self.names_all.append(curr_name)
                    else:
                        sub_vols_3d.append(patch_3d)
                        self.coords.append(positions[p])
                        self.names_all.append(curr_name)
