...
class ParticlePreProjDataset3D(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        num_of_augs = len(sub_vol_set_3d)
        curr_coord = self.coords[index]
        # print('sub_vol', sub_vol.shape)
        # p = np.random.rand()
        ...
        aug_2 = self.weak_transforms(aug_patch3d)
        # aug_2 = aug_patches

        # if p > 0.5:
        #     aug_patch = sub_vol_set[1]
        #     aug_2 = self.weak_transforms(aug_patch)
        # else:
        #     aug_patch = sub_vol_set[0]
        #     aug_2 = self.transforms(aug_patch)
        # print('aug patch', aug_patch.shape)
        # subvol_coords = self.coords[index]
        # rec_name = self.names_all[index]
        # used_v = self.tomos[rec_name]
        # used_angle = self.angles[rec_name]
        ...
        # tomo_x, tomo_y, tomo_z = subvol_coords[0], subvol_coords[1], subvol_coords[2]
        # x_c_r, y_c_r, z_c_r = tomo_x + off_x, tomo_y + off_y, tomo_z + off_z
        # aug_patch = None 
        # while aug_patch is None:
        #     aug_patch = self.extract_patches(used_v, [x_c_r, y_c_r, z_c_r], used_angle)
        # aug_patch -= aug_patch.min()
        # aug_patch /= aug_patch.max()
        # aug_patch = torch.tensor(aug_patch)
        # aug_patch = aug_patch.unsqueeze(0).float()
        ...
        # aug_2 = self.transforms(sub_vol)
        
        # aug_2 = self.transforms(sub_vol)
        aug_1 = self.weak_transforms(sub_vol3d)
        # aug_1 = sub_vols
        # tomo = self.tomos[index]
        # flip_prob1 = np.random.random()
        # flip_prob2 = np.random.random()
        # swap_prob = np.random.random()
        # d_prob1 = np.random.random()
        # d_prob2 = np.random.random()
        # drop1, drop2 = self.size[0]//8, self.size[1]//8
        # center1, center2 = self.size[0]//4, self.size[1]//4
        # if flip_prob1 < 0.33:
        #   aug_1 = flip_ud(self.transforms(sub_vol))
        # elif flip_prob1 > 0.66:
        #   aug_1 = flip_lr(self.transforms(sub_vol))
        # else:
        #   aug_1 = self.transforms(sub_vol)
        # if flip_prob2 < 0.33:
        #   aug_2 = flip_ud(self.transforms(sub_vol))
        # elif flip_prob2 > 0.66:
        #   aug_2 = flip_lr(self.transforms(sub_vol))
        # else:
        #   aug_2 = self.transforms(sub_vol)
        # # print('aug_1', aug_1.shape)
        # if d_prob1 < 0.25:
        #   # aug_1 = drop_out(aug_1, (2, 12, 12))
        #   aug_1 = drop_out(aug_1, (drop1, drop2, drop2))
        # if 0.25 <= d_prob1 < 0.5:
        #   # aug_1 = center_out(aug_1, (4, 18, 18))
        #   aug_1 = center_out(aug_1, (center1, center2, center2))
        # if 0.5 <= d_prob1 < 0.75:
        #   # aug_1 = swap_out(aug_1, (2, 12, 12))
        #   aug_1 = swap_out(aug_1, (drop1, drop2, drop2))

        
        # if d_prob2 < 0.25:
        #   # aug_2 = drop_out(aug_2, (2, 12, 12))
        #   aug_2 = drop_out(aug_2, (drop1, drop2, drop2))
        # if 0.25 <= d_prob2 < 0.5:
        #   # aug_2 = center_out(aug_2, (4, 18, 18))
        #   aug_2 = center_out(aug_2, (center1, center2, center2))
        # if 0.5 <= d_prob2 < 0.75:
        #   # aug_2 = swap_out(aug_2, (2, 12, 12))
        #   aug_2 = swap_out(aug_2, (drop1, drop2, drop2))
            
        # if swap_prob > 0.5:
        #   aug_2 = change_view(aug_2)

        # aug_2 = self.transforms(sub_vol)

        # crop_aug1 = self.crop(aug_1)
        # crop_aug2 = self.crop(aug_2)
        # hm = self.hms[index]
        # hm = self.hms[i]
        # ind = self.inds[index]
